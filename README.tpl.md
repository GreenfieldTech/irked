...
Sometime people make mistake and when Irked is asked to setup your controllers, it
will find mistakes - it happens. Irked will make every attempt to detect any Irked
routing configuration mistakes during the initial setup (when calling `with()` or
one of the other setup methods) and will throw an `InvalidRouteConfiguration` exception
with as much details as appropriate - before the HTTP server request handler is set
up. We make every attempt to not cause exceptions to be thrown during actual request
processing, and any such generated by the Irked implementation will be consideed bugs and will be fixed.
