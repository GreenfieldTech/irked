...
vertx.createHttpServer()
		.requestHandler(Irked.router(getVertx()).with(new com.example.api.Root())::accept)
		.listen(port, async);
```
...
that gets out of the way of the developer and allows them to leverate other Irked provided facilities to manage WebSocketsf,
as detailed blow.

I the case that the developer would like to gain lower level access to the WebSocket upgrade protocol, or use the more complex
...
you can always add the handler when initializing the server, before or after setting up Irked. Please that implementing
this method blocks any other WebSocket implementation strategy suggested by this document - the WebSocket Stream handler
...
WebHandler lisener = r -> {
  if (!r.needUpgrade("websocket")) r.fail(new BadRequest());
  else startConnection(r.request().upgrade());
};

private void startConnection(ServerWebSocket sock) {
  sock.handler(message -> {
   sock.write("Echo: " + message.toString().trim() + "\n");
  });
}
```
