...
Irked is available from the [Maven Central Repository](https://central.sonatype.com/artifact/tech.greenfield/irked-vertx/4.5.16).
...
boils down to just using `tech.greenfield:irked-vertx:4.5.16` as the dependency string.
...
To run the example application, after compiling (for example, using `mvn compile`) run it with your full Vert.x 4.5.16 installation:
